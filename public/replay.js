...
// ---- Pixel-art unit sprites (drawn in code, zero image assets) ----
// Each unit is painted on a 16x18 grid with rectangle primitives, then cached
// as a data URL. Team color (P = primary, p = shade) is swapped per side; side
// B is flipped to face left via CSS. Tweak the builders below to refine a unit.
const SPRITE_W = 16, SPRITE_H = 18;
const TEAM_COLORS = {
  a: { P: "#5b8fd6", p: "#33597f" },
  b: { P: "#d65b5b", p: "#7f3333" },
};
// Fixed (non-team) palette.
const PX = {
  o: "#15151f", // outline / boots
  k: "#3a3f4b", // dark steel / cap
  m: "#c6ccd8", // bright metal (blades, wrench)
  w: "#8a5a2b", // wood (shafts, bow)
  s: "#f2c9a0", // skin
  h: "#46321f", // hair
  a: "#ffcf4a", // gold accent / cross / hilt
  g: "#7fe6ff", // magic orb / glow
  W: "#eef0f4", // white robe
};

function spx(cx, x, y, w, h, c) { if (!c) return; cx.fillStyle = c; cx.fillRect(x, y, w, h); }

// Shared humanoid: head, team-colored torso, arms, legs, boots.
function drawBase(cx, P, p, opts) {
  opts = opts || {};
  if (opts.hair !== null) {
    const hair = opts.hair || PX.h;
    spx(cx, 6, 2, 4, 2, hair);
    spx(cx, 5, 3, 1, 2, hair);
    spx(cx, 10, 3, 1, 2, hair);
  }
  spx(cx, 6, 4, 4, 3, PX.s);   // face
  spx(cx, 8, 5, 1, 1, PX.o);   // eye
  spx(cx, 7, 7, 2, 1, PX.s);   // neck
  spx(cx, 5, 8, 6, 3, P);      // torso
  spx(cx, 4, 8, 1, 3, P);      // left arm
  spx(cx, 11, 8, 1, 3, P);     // right arm
  spx(cx, 4, 11, 1, 1, PX.s);  // left hand
  spx(cx, 11, 11, 1, 1, PX.s); // right hand
  spx(cx, 5, 11, 6, 1, p);     // belt
  spx(cx, 5, 12, 2, 3, p);     // left leg
  spx(cx, 9, 12, 2, 3, p);     // right leg
  spx(cx, 5, 15, 2, 1, PX.o);  // left boot
  spx(cx, 9, 15, 2, 1, PX.o);  // right boot
}

const SPRITE_BUILDERS = {
  knight(cx, P, p) {
    drawBase(cx, P, p);
    spx(cx, 12, 1, 1, 1, PX.m);   // sword tip
    spx(cx, 12, 2, 1, 7, PX.m);   // blade
    spx(cx, 11, 9, 3, 1, PX.a);   // guard
    spx(cx, 2, 7, 3, 5, PX.k);    // shield dark backing
    spx(cx, 2, 8, 3, 3, P);       // shield face (team)
    spx(cx, 2, 8, 3, 1, PX.a);    // shield rim top
    spx(cx, 3, 9, 1, 2, PX.m);    // shield boss
  },
  spear(cx, P, p) {
    drawBase(cx, P, p);
    spx(cx, 12, 3, 1, 9, PX.w);   // shaft
    spx(cx, 12, 0, 1, 1, PX.m);   // spearhead tip
    spx(cx, 11, 1, 3, 2, PX.m);   // spearhead solid block
  },
  archer(cx, P, p) {
    drawBase(cx, P, p);
    spx(cx, 13, 3, 1, 8, PX.w);   // bow limb
    spx(cx, 12, 3, 1, 1, PX.w);   // top nock
    spx(cx, 12, 10, 1, 1, PX.w);  // bottom nock
    spx(cx, 12, 4, 1, 6, PX.k);   // string
    spx(cx, 9, 6, 4, 1, PX.w);    // arrow shaft
    spx(cx, 13, 6, 1, 1, PX.m);   // arrowhead
  },
  mage(cx, P, p) {
    drawBase(cx, P, p, { hair: null });
    spx(cx, 8, 0, 1, 1, P);       // hat tip
    spx(cx, 7, 1, 3, 1, P);
    spx(cx, 6, 2, 5, 1, P);
    spx(cx, 5, 3, 6, 1, p);       // hat brim
    spx(cx, 12, 4, 1, 8, PX.w);   // staff
    spx(cx, 11, 2, 3, 2, PX.g);   // orb
    spx(cx, 12, 1, 1, 1, PX.g);   // orb glow
  },
  priest(cx, P, p) {
    spx(cx, 6, 4, 4, 3, PX.s);    // face
    spx(cx, 8, 5, 1, 1, PX.o);    // eye
    spx(cx, 5, 2, 6, 2, P);       // hood top
    spx(cx, 5, 4, 1, 3, P);       // hood side
    spx(cx, 10, 4, 1, 3, P);      // hood side
    spx(cx, 7, 7, 2, 1, PX.s);    // neck
    spx(cx, 5, 8, 6, 3, PX.W);    // white robe torso
    spx(cx, 4, 8, 1, 3, PX.W);    // sleeves
    spx(cx, 11, 8, 1, 3, PX.W);
    spx(cx, 5, 8, 6, 1, p);       // team trim
    spx(cx, 8, 9, 1, 3, PX.a);    // chest cross (vert)
    spx(cx, 7, 10, 3, 1, PX.a);   // chest cross (horiz)
    spx(cx, 4, 11, 8, 4, PX.W);   // robe skirt
    spx(cx, 4, 14, 8, 1, p);      // hem
    spx(cx, 13, 3, 1, 9, PX.w);   // staff
    spx(cx, 13, 1, 1, 2, PX.a);   // staff cross (vert)
    spx(cx, 12, 2, 3, 1, PX.a);   // staff cross (horiz)
  },
  engineer(cx, P, p) {
    drawBase(cx, P, p, { hair: null });
    spx(cx, 6, 2, 4, 1, PX.k);    // cap top
    spx(cx, 5, 3, 5, 1, PX.k);    // cap band
    spx(cx, 10, 3, 2, 1, PX.k);   // cap brim
    spx(cx, 11, 5, 1, 6, PX.m);   // wrench handle
    spx(cx, 10, 4, 3, 2, PX.m);   // wrench head
    spx(cx, 11, 4, 1, 1, PX.o);   // wrench notch
  },
};

const _spriteCache = {};
function spriteURL(type, side) {
  const s = (side || "A").toLowerCase();
  const key = type + "_" + s;
  if (_spriteCache[key]) return _spriteCache[key];
  const team = TEAM_COLORS[s] || TEAM_COLORS.a;
  const cv = document.createElement("canvas");
  cv.width = SPRITE_W;
  cv.height = SPRITE_H;
  const cx = cv.getContext("2d");
  (SPRITE_BUILDERS[type] || SPRITE_BUILDERS.knight)(cx, team.P, team.p);
  const url = cv.toDataURL();
  _spriteCache[key] = url;
  return url;
}
