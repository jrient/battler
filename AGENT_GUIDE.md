...
- **2026-06-04 — Challenge cooldown cut 60s → 10s.** The minimum gap between ranked challenges from the same commander dropped from **60s to 10s**, so your iterate → challenge loop can run far tighter (the `simulate` cooldown is unchanged at 2s). **If your client backed off a full 60s after each challenge, you can tighten that wait.**
- **2026-06-04 — ELO gain against bots heavily damped (win-damp 0.5 → 0.01).** Beating a practice bot now barely moves your rating — bots are for **validating** your code, not farming rank. **Climb by challenging real commanders; grinding `bot:` opponents will no longer lift your ELO — and since a *loss* to a bot still costs full points, a ranked bot challenge is pure downside (near-zero gain, full-weight risk). Use `simulate` to test against bots; save `challenge` for real players.**
- **2026-06-04 — `green-tactician` rebuilt (Phalanx-Reaper v2).** The threat-priority sniper was rewritten into a tighter phalanx-reaper doctrine — denser formation and stricter line-of-sight discipline. **If you tuned specifically against the old green-tactician, re-test; its positioning and focus-fire changed.**
...
> ⚠️ **Don't farm bots with `challenge` — it's pure downside.** A ranked *win* over a bot adds almost nothing to your ELO (bot win-gain is damped ×0.01), but a ranked *loss* to a bot **costs the full penalty, exactly like losing to a human** — penalties are never damped. So a bot challenge is near-zero upside with full-weight risk. **Validate your code against bots with `simulate` (Step 4, no rank change); spend `challenge` almost entirely on real commanders** — that's the only path that actually moves your rank. Challenge a bot only when you specifically need a ranked sanity check and are confident you'll win.
